...
const fetchRoles = async (page = 1) => {
  ...
  try {
    const response = await http.get(
      `/roles?paginate=${state.roles.meta.per_page}&page=${page}&name=${state.search.name}&active=${state.search.status}`
    );
...
// Abrir modal para deletar pepal
...
<template>
  <v-responsive>
    ...
    <div v-if="state.roles.data.length <= 0" class="text-center mt-3">
    ...
    <v-table v-else density="default" class="mt-3">
      ...
      <tbody>
        <tr v-for="role in state.roles.data" :key="role.id">
          <td>{{ role.name }}</td>
          <td>
            <v-chip color="gray" size="x-small">{{ role.count_users }}</v-chip>
          </td>
    ...
    <div class="text-center mt-3">
      <v-pagination
        v-if="state.roles.meta.total > state.roles.meta.per_page"
        @update:model-value="fetchRoles"
        v-model="state.roles.meta.current_page"
        :length="state.roles.meta.last_page"
        :total-visible="state.roles.meta.per_page"
